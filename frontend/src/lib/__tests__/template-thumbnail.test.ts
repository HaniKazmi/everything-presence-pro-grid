...
function makeGrid(insideCells: { col: number; row: number; zone?: number }[]): number[] {
  const grid = new Array(GRID_CELL_COUNT).fill(0);
  for (const { col, row, zone } of insideCells) {
    grid[row * GRID_COLS + col] = CELL_ROOM_BIT | ((zone ?? 0) << CELL_ZONE_SHIFT);
  }
  return grid;
}
...
describe("renderTemplateThumbnail", () => {
  it("renders svg with rect elements for inside cells", async () => {
    ...
    const result = renderTemplateThumbnail(grid, new Array(7).fill(null), 600, 600, []);
  ...
  it("returns empty svg when grid has no inside cells", async () => {
    const grid = new Array(GRID_CELL_COUNT).fill(0);
    const result = renderTemplateThumbnail(grid, new Array(7).fill(null), 3000, 3000, []);
