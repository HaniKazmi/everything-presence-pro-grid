"""Runtime BLE-scan toggle for the wifi-ble-co2 variant.

The bluetooth_proxy + 100%-duty-cycle active BLE scan combination is the
dominant heap consumer on this firmware (~30-80 KB resident, with transient
spikes during scan-result processing). Users who don't actually have BLE
devices to proxy still pay the full heap cost, leaving them on the OOM
edge during simultaneous network load (TLS handshake, OTA download).
...
State must persist across reboots so a deliberate "off" survives an OTA
or power cycle.

Knock-on effect: the OTA-error hook in this same file unconditionally
calls `esp32_ble_tracker.start_scan` to restore scan after a failed OTA.
That'd accidentally re-enable the scan for users who'd toggled it off.
The hook needs to check the toggle state and only restart if the user
had it on.
"""
