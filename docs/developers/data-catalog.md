...
| WiFi Disconnects       | sensor        | drops since boot, counted on the connected→disconnected edge (wifi variant) |
| WiFi Disconnect Reason | text_sensor   | IDF reason code + name for the last drop, e.g. `Beacon Timeout (200)`       |
| WiFi Disconnect Signal | sensor        | RSSI in dBm at the instant of the last drop (from the IDF event)            |
| WiFi Downtime          | sensor        | seconds the last WiFi outage lasted (published on reconnect)                |
