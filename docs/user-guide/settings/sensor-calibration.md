...
The motion and static-presence settings here also feed into the [sensor-assisted clear](../how-detection-works.md#sensor-assisted-clear) — the rule that lets the zone engine drop *pending* zones early once both hardware sensors report inactive. So lowering a timeout doesn't just affect that one sensor's entity — it can shorten how long an empty room stays marked occupied.
...
The SEN0609 mmWave radar reports binary presence — "someone here" or "not here". Range is set under [Detection ranges](detection-ranges.md); the controls here govern the chip's own sensitivity and the timing around its output.
...
The Trigger and Renew thresholds here are the SEN0609's own internal scale, not the zone-level Trigger and Renew described in [How detection works](../how-detection-works.md#the-zone-state-machine). They control how easily the radar chip itself decides "yes, presence" — separate from how the zone engine decides "yes, this zone is occupied".
...
Fixed offsets that nudge the on-board temperature, humidity, and illuminance readings to match a reference instrument. The device runs warm and is usually mounted high in a corner, so temperature and humidity readings drift accordingly. An offset gets the average closer to truth, but it can't fix airflow, sun on the casing, or the device's own self-heating curve. Treat the readings as trends with a calibrated baseline, not as climate-grade measurements. See [Hardware → Environmental sensors](../hardware.md#environmental-sensors).

!!! note "CO₂ module"
    The optional CO₂ add-on isn't calibrated from this page — there's no offset slider for it. To fit the module and run its forced-recalibration routine, follow the [CO₂ module integration guide](https://docs.everythingsmart.io/s/products/doc/integrate-the-carbon-dioxide-co2-module-biegKGfCWu).
...
1. Place a known-good thermometer / hygrometer / lux meter near the sensor — same spot, same airflow, same minute. Wait for both to stabilise.
...
!!! note
    A single fixed offset is a rough correction. The self-heating curve depends on ambient temperature and on how much radio traffic the device is doing, so an offset that's right at 20 °C ambient won't be exactly right at 28 °C. Aim for "close enough across normal conditions" rather than a perfect match at one moment.
