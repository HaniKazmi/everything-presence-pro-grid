...
|-----|------|-----------|---------|
| `relay_trigger_mode` | string | `vol.In(["disabled", "motion", "presence", "motion_or_presence"])` | `"disabled"` |
...
- `trigger_mode`: `"disabled"`, `"motion"`, `"presence"`, `"motion_or_presence"`
- `contact_mode`: `"no"`, `"nc"`
...
1. Determine activation based on trigger mode:
   - `disabled` -> `false`
   - `motion` -> `result.motion_state != INACTIVE`
   - `presence` -> `result.occupancy`
   - `motion_or_presence` -> either of the above
2. Apply contact mode:
   - `no` (Normally Open) -> activation as-is
   - `nc` (Normally Closed) -> invert activation
3. Call `turn_on()`/`turn_off()` on the `system_alarm_relay` switch entity (not raw GPIO) so the ESPHome switch entity state stays in sync with the actual pin state

**Manual override**: If a user manually toggles the relay switch in HA, the automatic logic will override it on the next 1Hz evaluation tick. This is intentional — the settings control automatic behavior; manual toggling is for testing only.
...
- Add validation to `set_settings` schema:
  - `vol.Required("relay_trigger_mode"): vol.In(["disabled", "motion", "presence", "motion_or_presence"])`
...
```typescript
relayTriggerMode: string;  // "disabled" | "motion" | "presence" | "motion_or_presence"
...
- **Trigger Mode** select: Disabled, Motion Only, Presence Only, Motion or Presence
