...
async def websocket_overview_subscribe_heatmap(
    ...
    try:
        ...
        _LOGGER.warning("overview/subscribe_heatmap: open session failed for %s: %s", mac, err)
        device_conn = None
    if device_conn is None:
        connection.send_message(websocket_api.event_message(msg["id"], {"available": False}))
        return
    ...
    try:
        on_state = _make_heatmap_on_state(connection, msg["id"], mac, device_conn)
        await device_conn.subscribe_states(on_state)
    except Exception as err:
        _LOGGER.warning("overview/subscribe_heatmap: subscribe failed for %s: %s", mac, err)
        manager.release_session(mac, device_conn)
        connection.send_message(websocket_api.event_message(msg["id"], {"available": False}))
        return
    manager.note_target_subscribe(mac, "heatmap_subs")
    hass.async_create_task(manager.async_push_pipeline_to_device(mac))
    ...
    def _unsub() -> None:
        ...
        mgr = _get_manager(hass)
        if mgr:
            mgr.note_target_unsubscribe(mac, "heatmap_subs")
            hass.async_create_task(mgr.async_push_pipeline_to_device(mac))
            mgr.release_session(mac, device_conn)
